			...
			<BntCardBody>
				<BntStack direction="column">
					...
					<BntBox>
						<BntTypography variant="caption2">
							<>
								{`${scheduler.every === TSchedulerType.weekly ? t(getWeekdayOptions().find((x) => x.id === scheduler.weekday)?.label) : `${scheduler.day}  ${t(texts_d.day)}`}`}
								{` ${formatStringDate(secondsToTime(scheduler.time_in_seconds), false, true, undefined, true, false)} ${scheduler.timezone ? `[${scheduler.timezone}]` : CommonStrings.EMPTY_STRING}`}
							</>
						</BntTypography>
					</BntBox>
				</BntStack>
