...
export const RequestFeedItem: FC<RequestFeedItemProps> = ({ isMyRequestView = false, primaryAction, request, secondaryAction }) => {
	...
	const { showEmployee } = useEmployeeUi(request.profile);
				...
				{isMyRequestView ? (
					<BntTransparentButton disableRipple onClick={() => showDonut(request.donut?.id)} sx={{ justifyContent: "flex-start", p: 0, minWidth: 0, width: "fit-content" }}>
						<Stack direction="row" gap={1.5} alignItems="center" sx={{ minWidth: 0, flex: 1 }}>
							...
							<Stack gap={0.5} sx={{ minWidth: 0, flex: 1 }}>
								<BntTypography variant="h6" fontWeight={700} sx={{ color: "text.primary", textAlign: "left" }}>
									{donutName}
								</BntTypography>
					...
					<>
						<Stack direction="row" gap={1.5} alignItems="flex-start" sx={{ minWidth: 0, flex: 1 }}>
							{request.profile ? (
								<BntTransparentButton disableRipple onClick={() => showEmployee(request.profile?.id)} sx={{ justifyContent: "flex-start", p: 0, minWidth: 0 }}>
									<Stack direction="row" gap={1.5} alignItems="flex-start" sx={{ minWidth: 0, flex: 1 }}>
										...
										<Stack gap={0.5} sx={{ minWidth: 0, flex: 1 }}>
											<BntTypography variant="subtitle1" fontWeight={700} sx={{ color: "text.primary", textAlign: "left" }}>
												{displayName}
											</BntTypography>
											{position ? (
												<BntTypography variant="body2" sx={{ color: "text.secondary", textAlign: "left" }}>
													{position}
												</BntTypography>
											) : null}
											{email ? (
												<BntTypography variant="body2" sx={{ color: "text.secondary", overflowWrap: "anywhere", textAlign: "left" }}>
													{email}
												</BntTypography>
											) : null}
										</Stack>
						...
						{request.donut ? (
							<DonutButton donut={request.donut} onClick={() => showDonut(request.donut?.id)} />
						) : donutName ? (
							<BntTypography variant="body2" sx={{ color: "text.secondary" }}>
								{donutName}
							</BntTypography>
